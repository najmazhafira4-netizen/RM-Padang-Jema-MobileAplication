...
// ==========================================
// DATABASE AKUN LOKAL (SIMULASI)
// ==========================================
// Dipindahkan ke tingkat global agar data pendaftaran tersimpan selama aplikasi berjalan.
...
// ==========================================
// HALAMAN: LOGIN MULTI-USER + GOOGLE AUTH
// ==========================================
...
class _HalamanLoginState extends State<HalamanLogin> {
  ...
  void _prosesLogin() {
    ...
    Future.delayed(const Duration(milliseconds: 800), () {
      ...
      final pencarian = usersDatabase.firstWhere(
        (u) => u['username']?.toLowerCase() == user.toLowerCase() && u['password'] == pass,
        orElse: () => {},
      );

      if (pencarian.isNotEmpty) {
        String role = pencarian['role'] ?? 'pelanggan';
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Login Berhasil sebagai ${role.toUpperCase()}!'), backgroundColor: Colors.green),
        );

        if (role == 'admin' || role == 'kasir') {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(builder: (context) => const HalamanRiwayat(isAdminView: true)),
          );
        } else {
          // Navigasi jika pelanggan kustom login via username & password
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(builder: (context) => const HalamanSampul()),
          );
        }
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Akun tidak ditemukan atau sandi salah!'), backgroundColor: Colors.red),
        );
      }
    });
  }
  ...
  void _prosesLoginGoogle() {
    ...
    // Simulasi respons API Google Sign-In
...
class _HalamanRegistrasiState extends State<HalamanRegistrasi> {
  ...
  void _prosesRegistrasi() {
    ...
    if (user.isEmpty || pass.isEmpty || konfirmasiPass.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    bool userAda = usersDatabase.any((u) => u['username']?.toLowerCase() == user.toLowerCase());
    if (userAda) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Username sudah terdaftar! Pilih nama lain.'), backgroundColor: Colors.red),
      );
      return;
    }
    ...
    Future.delayed(const Duration(milliseconds: 1000), () {
      ...
      // Tambahkan user baru ke database tiruan
      usersDatabase.add({
        'username': user,
        'password': pass,
        'role': 'pelanggan'
...
class _HalamanBerandaState extends State<HalamanBeranda> {
  ...
  final List<Map<String, dynamic>> menuDessert = [
    ...
    {'hover': 'Klepon', 'nama': 'Klepon', 'harga': 'Rp 10.000', 'gambar': 'https://assets.pikiran-rakyat.com/crop/0x0:0x0/1200x675/webp/photo/2024/06/16/1633124760.jpg'},
  ...
  Future<void> _fetchStok() async {
    const String url = 'https://script.google.com/macros/s/AKfycbxqFaFGxlQYtjM0FtHTWt5Pp8SwP6VRPm30qkqdlbaxa7rC4LGrwJAeT1EX8RcJBPNiVg/exec'; 
    try {
...
// ==========================================
// HALAMAN 3: DASHBOARD ADMIN & KASIR (DENGAN LOGOUT)
// ==========================================
class HalamanRiwayat extends StatefulWidget {
  final bool isAdminView;
  const HalamanRiwayat({super.key, this.isAdminView = true});
...
class _HalamanRiwayatState extends State<HalamanRiwayat> {
  ...
  Future<void> _fetchRiwayat() async {
    const String url = 'https://script.google.com/macros/s/AKfycbxqFaFGxlQYtjM0FtHTWt5Pp8SwP6VRPm30qkqdlbaxa7rC4LGrwJAeT1EX8RcJBPNiVg/exec?p=riwayat'; 
    try {
      final response = await http.get(Uri.parse(url));
      if (response.statusCode == 200) {
        setState(() {
  ...
  void _bukaHalamanLaporan(String tipe) {
    ...
    for (var pesanan in _riwayat) {
      ...
      try {
        ...
        waktuPesanan = DateTime.tryParse(stringWaktu);
        ...
        if (tipe == 'Harian') {
          masukKriteria = (waktuPesanan.year == sekarang.year &&
              waktuPesanan.month == sekarang.month &&
              waktuPesanan.day == sekarang.day);
      ...
      builder: (context) {
        return Container(
          ...
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('Pilih Periode Laporan Keuangan', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const SizedBox(height: 15),
              ListTile(
              ...
              ListTile(
                ...
                title: const Text('Laporan Mingguan (7 Hari Terakhir)'),
  ...
  void _logoutDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Apakah Anda yakin ingin keluar dari halaman admin/kasir ini?'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text('BATAL')),
          TextButton(
            onPressed: () {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.isAdminView ? 'Dashboard Admin - Riwayat' : 'Riwayat Pesanan'),
        backgroundColor: const Color.fromARGB(255, 174, 23, 23),
        foregroundColor: Colors.white,
        actions: [
          if (widget.isAdminView) ...[
            IconButton(
              ...
              onPressed: _pilihJenisLaporan,
            ),
            IconButton(
              icon: const Icon(Icons.logout),
              tooltip: 'Logout Sistem',
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(builder: (context) => HalamanStruk(dataPesanan: dataPesanan)),
                          );
                        },
...
// ==========================================
// HALAMAN: RENDER TABEL RINCIAN DETAIL LAPORAN
// ==========================================
class HalamanDetailLaporan extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (var pesanan in daftarPesanan) {
      var totalBayar = pesanan['Total'] ?? pesanan['total'] ?? pesanan['Total_Bayar'] ?? pesanan['Total_bayar'] ?? 0;
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Laporan Periode $tipeLaporan'),
        backgroundColor: const Color.fromARGB(255, 174, 23, 23),
        foregroundColor: Colors.white,
      ),
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text(
              "Rincian Riwayat Transaksi Tabel", 
              style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color.fromARGB(255, 174, 23, 23))
            ),
                ...
                child: Column(
                  children: [
                    ...
                    Container(
                      ...
                      child: Text(
                        "${dataPesanan['Menu'] ?? dataPesanan['menu'] ?? 'Tidak ada detail menu'}", 
...
// ==========================================
// HALAMAN 2: CHECKOUT & PROSES TRANSAKSI
// ==========================================
...
class _HalamanCheckoutState extends State<HalamanCheckout> {
  ...
  final Map<String, String> infoPembayaran = {
    'GoPay': '0812-3456-7890 (Najwa Zhafira Nofika)',
    'DANA': '0812-3456-7890 (Najwa Zhafira Nofika)',
    'Bank BRI': '1234-01-000123-50-1 (Najwa Zhafira Nofika)',
    'Bank BSI': '7123456789 (Najwa Zhafira Nofika)',
  ...
  Future<void> kirimPesanan() async {
    ...
    setState(() => _isLoading = true);
    const String urlGAS = 'https://script.google.com/macros/s/AKfycbxqFaFGxlQYtjM0FtHTWt5Pp8SwP6VRPm30qkqdlbaxa7rC4LGrwJAeT1EX8RcJBPNiVg/exec';
    
    try {
      String teksMenu = widget.isiKeranjang.map((i) => "${i['jumlah']}x ${i['nama']}").join(", ");
      String waktuSekarang = DateTime.now().toString().split('.')[0]; 
      ...
      Map<String, dynamic> data = {
        ...
        "total": widget.isiKeranjang.fold(0, (sum, item) => sum + (item['hargaInt'] as int) * (item['jumlah'] as int)),
        "waktu": waktuSekarang 
      };

      final response = await http.post(Uri.parse(urlGAS), body: jsonEncode(data));
